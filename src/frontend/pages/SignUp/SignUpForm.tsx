...
import {setSetting} from '../../utils/API';
...
export default function SignUpForm(): React.JSX.Element {
  ...
  const handleSignUp = async (e: Event): Promise<void> => {
    ...
    try {
      ...
      if (created && created === formState.username) {
        await setSetting('username', formState.username);
        await setSetting('email', formState.email);
        await setSetting('deviceID', formState.deviceID);
        await setSetting('password', formState.password);
        // default to auto login to true
        await setSetting('AUTO_LOGIN', 'true');
        // enable auto login
  ...
  useEffect(() => {
    ...
    validatedUsername.validated,
    validateEncryptionPassword.validated,
    validatedDeviceID.validated,
  ]);
    ...
    if (isMounted) {
      ...
      if (validateEncryptionPassword.error.length > 0 && formState.encryptionPassword !== '') {
        setEncryptionPasswordErrors(
          validateEncryptionPassword.error.map(error => Object.values(error)[0]),
        );
      } else {
        setEncryptionPasswordErrors([]);
      }
