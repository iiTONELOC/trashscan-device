 ...
 */
export const userSuccessfullyDecrypted = (user: IUser | IUserModel): boolean =>
  validators.isEmail(user?.email ?? '');
...
async function updateUser(
  ...
  // istanbul ignore next
  const hasUpdateData = Boolean(
    updateData?.username || updateData?.email || updateData?.password || updateData?.deviceID,
  );
  // istanbul ignore next
  const usernameHasLength = Boolean(
    updateData?.username?.length && updateData?.username?.length > 0,
  );
  // istanbul ignore next
  const emailHasLength = Boolean(updateData?.email?.length && updateData?.email?.length > 0);
  // istanbul ignore next
  const passwordHasLength = Boolean(
    updateData?.password?.length && updateData?.password?.length > 0,
  );
  const hasDeviceID = Boolean(updateData?.deviceID?.length && updateData?.deviceID?.length > 0);
  // istanbul ignore next
  const updatedDataHasLength = Boolean(
    usernameHasLength || emailHasLength || passwordHasLength || hasDeviceID,
  );
...
async function changeEncryptionPassword(
  ...
  const appSettings: IAppSettingsModel['settings'] = models?.AppSettings
    .settings as IAppSettingsModel['settings'];
  ...
  for (const setting of currentSettings) {
    ...
    const decryptedSetting: ISettings = await decryptAppSetting(
...
export {create, getUserBy, updateUser, readUsers, deleteUser, changeEncryptionPassword};
