...
export const autoLogin = {
  enable: (encryptionKey: Buffer, currentUser: IUserEncrypted) => {
    ...
    if (autoLoginKeyLine) {
      ...
      const newEnvs = envs
        .map(env => (env === autoLoginKeyLine ? newAutoLoginKeyLine : env))
    ...
    if (autoLoginUserLine) {
      ...
      const newEnvs = envs
        .map(env => (env === autoLoginUserLine ? newAutoLoginUserLine : env))
        .filter(env => env.length);
      fs.writeFileSync(_envFilePath, newEnvs.join('\n') + '\n');
    } else {
      // if it doesn't exist, add it to the end of the file
  ...
  autoLoginUser: async (
    ...
    const decryptedSuccessfully =
      decryptedUser.email.includes('@') && validators.isEmail(decryptedUser.email);
